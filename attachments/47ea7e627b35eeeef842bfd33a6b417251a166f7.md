# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: ui/salesforce.spec.ts >> Lead > Opportunity Conversion @smoke
- Location: tests/ui/salesforce.spec.ts:87:5

# Error details

```
Error: expect(locator).toBeVisible() failed

Locator: getByText('was deleted.')
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('was deleted.')

```

```yaml
- link "Skip to Navigation":
  - /url: javascript:void(0);
- link "Skip to Main Content":
  - /url: javascript:void(0);
- button "Toggle Panel": Menu
- text: Developer Edition
- button "Show menu"
- button "Search": Search...
- navigation "Global Header":
  - list:
    - listitem:
      - group:
        - button "Add favorite":
          - tooltip "Add favorite"
        - button "Favorites list":
          - tooltip "Favorites list"
    - listitem:
      - button "Global Actions":
        - tooltip "Global Actions"
    - listitem:
      - button "Guidance Center":
        - tooltip "Guidance Center"
    - listitem:
      - button "Salesforce Help":
        - tooltip "Salesforce Help"
    - listitem:
      - button "Setup":
        - tooltip "Setup"
    - listitem:
      - button "Notifications":
        - tooltip "Notifications"
    - listitem:
      - button "View profile":
        - tooltip "View profile"
- navigation "App":
  - button "App Launcher"
- heading "Sales" [level=1]
- navigation "Global":
  - list:
    - listitem:
      - link "Home":
        - /url: /lightning/page/home
    - listitem:
      - link "Opportunities":
        - /url: /lightning/o/Opportunity/home
      - button "Opportunities List"
    - listitem:
      - link "Leads":
        - /url: /lightning/o/Lead/home
      - button "Leads List"
    - listitem:
      - link "Tasks":
        - /url: /lightning/o/Task/home
      - button "Tasks List"
    - listitem:
      - link "Files":
        - /url: /lightning/o/ContentDocument/home
      - button "Files List"
    - listitem:
      - link "Accounts":
        - /url: /lightning/o/Account/home
      - button "Accounts List"
    - listitem:
      - link "Contacts":
        - /url: /lightning/o/Contact/home
      - button "Contacts List"
    - listitem:
      - link "Campaigns":
        - /url: /lightning/o/Campaign/home
      - button "Campaigns List"
    - listitem:
      - link "Dashboards":
        - /url: /lightning/o/Dashboard/home
      - button "Dashboards List"
    - listitem:
      - button "Show more navigation items"
    - listitem:
      - button "Edit nav items"
- main:
  - heading "Opportunities" [level=1]
  - heading "Opportunities Recently Viewed" [level=1]
  - 'button "Select a List View: Opportunities"'
  - button "This list is pinned." [disabled]
  - list:
    - listitem:
      - button "New"
    - listitem:
      - button "Assign Label"
  - paragraph:
    - status "Recently Viewed": 8 items •
    - text: Updated a few seconds ago
  - text: Search this list...
  - searchbox "Search this list..."
  - button "List View Controls"
  - button "Select list display"
  - button "Refresh"
  - button "Column sort"
  - button "Edit List"
  - group:
    - button "Charts" [disabled]
    - button "Filters" [disabled]
  - text: Navigation Mode
  - grid:
    - rowgroup:
      - row "Row Number Choose a Row Select 8 items Opportunity Name Account Name Account Site Stage Close Date Opportunity Owner Alias Action":
        - columnheader "Row Number"
        - gridcell "Choose a Row Select 8 items":
          - text: Choose a Row
          - checkbox "Select 8 items"
          - text: Select 8 items
        - columnheader "Opportunity Name":
          - 'button "Sort by: Opportunity Name"'
          - text: "Sorted: None"
          - button "Show Opportunity Name column actions"
          - slider "Opportunity Name column width": "340"
        - columnheader "Account Name":
          - 'button "Sort by: Account Name"'
          - text: "Sorted: None"
          - button "Show Account Name column actions"
          - slider "Account Name column width": "334"
        - columnheader "Account Site":
          - 'button "Sort by: Account Site"'
          - text: "Sorted: None"
          - button "Show Account Site column actions"
          - slider "Account Site column width": "60"
        - columnheader "Stage":
          - 'button "Sort by: Stage"'
          - text: "Sorted: None"
          - button "Show Stage column actions"
          - slider "Stage column width": "159"
        - columnheader "Close Date":
          - 'button "Sort by: Close Date"'
          - text: "Sorted: None"
          - button "Show Close Date column actions"
          - slider "Close Date column width": "145"
        - columnheader "Opportunity Owner Alias":
          - 'button "Sort by: Opportunity Owner Alias"'
          - text: "Sorted: None"
          - button "Show Opportunity Owner Alias column actions"
          - slider "Opportunity Owner Alias column width": "68"
        - columnheader "Action"
    - rowgroup:
      - row "Select Item 1 Choose a Row Luettgen Inc- Edit Opportunity Name Luettgen Inc Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 1 Choose a Row":
          - checkbox "Select Item 1 Choose a Row"
          - text: Select Item 1
        - rowheader "Luettgen Inc- Edit Opportunity Name":
          - link "Luettgen Inc-":
            - /url: /lightning/r/006g5000005S4dhAAC/view
          - button "Edit Opportunity Name"
        - gridcell "Luettgen Inc Edit Account Name":
          - link "Luettgen Inc":
            - /url: /lightning/r/001g500000TqCWQAA3/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 2 Choose a Row Gleichner, Keebler and Pacocha- Edit Opportunity Name Gleichner, Keebler and Pacocha Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 2 Choose a Row":
          - checkbox "Select Item 2 Choose a Row"
          - text: Select Item 2
        - rowheader "Gleichner, Keebler and Pacocha- Edit Opportunity Name":
          - link "Gleichner, Keebler and Pacocha-":
            - /url: /lightning/r/006g5000005S4aTAAS/view
          - button "Edit Opportunity Name"
        - gridcell "Gleichner, Keebler and Pacocha Edit Account Name":
          - link "Gleichner, Keebler and Pacocha":
            - /url: /lightning/r/001g500000TqZozAAF/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 3 Choose a Row Waters LLC- Edit Opportunity Name Waters LLC Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 3 Choose a Row":
          - checkbox "Select Item 3 Choose a Row"
          - text: Select Item 3
        - rowheader "Waters LLC- Edit Opportunity Name":
          - link "Waters LLC-":
            - /url: /lightning/r/006g5000005Rxe7AAC/view
          - button "Edit Opportunity Name"
        - gridcell "Waters LLC Edit Account Name":
          - link "Waters LLC":
            - /url: /lightning/r/001g500000Tpo2DAAR/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 4 Choose a Row Olson - Fritsch- Edit Opportunity Name Olson - Fritsch Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 4 Choose a Row":
          - checkbox "Select Item 4 Choose a Row"
          - text: Select Item 4
        - rowheader "Olson - Fritsch- Edit Opportunity Name":
          - link "Olson - Fritsch-":
            - /url: /lightning/r/006g5000005S2C5AAK/view
          - button "Edit Opportunity Name"
        - gridcell "Olson - Fritsch Edit Account Name":
          - link "Olson - Fritsch":
            - /url: /lightning/r/001g500000TqN6HAAV/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 5 Choose a Row Schultz - Rempel- Edit Opportunity Name Schultz - Rempel Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 5 Choose a Row":
          - checkbox "Select Item 5 Choose a Row"
          - text: Select Item 5
        - rowheader "Schultz - Rempel- Edit Opportunity Name":
          - link "Schultz - Rempel-":
            - /url: /lightning/r/006g5000005S22PAAS/view
          - button "Edit Opportunity Name"
        - gridcell "Schultz - Rempel Edit Account Name":
          - link "Schultz - Rempel":
            - /url: /lightning/r/001g500000TqMN7AAN/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 6 Choose a Row Reilly Group- Edit Opportunity Name Reilly Group Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 6 Choose a Row":
          - checkbox "Select Item 6 Choose a Row"
          - text: Select Item 6
        - rowheader "Reilly Group- Edit Opportunity Name":
          - link "Reilly Group-":
            - /url: /lightning/r/006g5000005S05RAAS/view
          - button "Edit Opportunity Name"
        - gridcell "Reilly Group Edit Account Name":
          - link "Reilly Group":
            - /url: /lightning/r/001g500000TqBrVAAV/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 7 Choose a Row Reichel - Runte- Edit Opportunity Name Reichel - Runte Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 7 Choose a Row":
          - checkbox "Select Item 7 Choose a Row"
          - text: Select Item 7
        - rowheader "Reichel - Runte- Edit Opportunity Name":
          - link "Reichel - Runte-":
            - /url: /lightning/r/006g5000005RVODAA4/view
          - button "Edit Opportunity Name"
        - gridcell "Reichel - Runte Edit Account Name":
          - link "Reichel - Runte":
            - /url: /lightning/r/001g500000TpUUoAAN/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
      - row "Select Item 8 Choose a Row Champlin Group- Edit Opportunity Name Champlin Group Edit Account Name Locked Account Site Prospecting Edit Stage 6/30/2026 Edit Close Date cng Locked Opportunity Owner Alias Show Actions":
        - gridcell
        - gridcell "Select Item 8 Choose a Row":
          - checkbox "Select Item 8 Choose a Row"
          - text: Select Item 8
        - rowheader "Champlin Group- Edit Opportunity Name":
          - link "Champlin Group-":
            - /url: /lightning/r/006g5000005R6MbAAK/view
          - button "Edit Opportunity Name"
        - gridcell "Champlin Group Edit Account Name":
          - link "Champlin Group":
            - /url: /lightning/r/001g500000TnRG5AAN/view
          - button "Edit Account Name"
        - gridcell "Locked Account Site":
          - img "Locked Account Site"
        - gridcell "Prospecting Edit Stage":
          - text: Prospecting
          - button "Edit Stage"
        - gridcell "6/30/2026 Edit Close Date":
          - text: 6/30/2026
          - button "Edit Close Date"
        - gridcell "cng Locked Opportunity Owner Alias":
          - link "cng":
            - /url: /lightning/r/005g5000008nNjdAAE/view
          - img "Locked Opportunity Owner Alias"
        - gridcell "Show Actions":
          - button "Show Actions"
  - status
- contentinfo "Utility Bar":
  - list:
    - listitem:
      - button "To Do List"
- status
```

# Test source

```ts
  79  |   }
  80  | 
  81  |   async verifyDashboardPage() {
  82  |     await expect(this.textFieldGlobalSearchBar).toBeVisible();
  83  |     await expect(this.buttonAppLauncher).toBeVisible();
  84  |   }
  85  | 
  86  |   async goToSalesPage() {
  87  |     await this.page.goto(
  88  |       "https://orgfarm-b21abd5166-dev-ed.develop.lightning.force.com/lightning/n/devedapp__Welcome"
  89  |     );
  90  |     await this.page.waitForLoadState("domcontentloaded");
  91  |   }
  92  | 
  93  |   async clickNewButton() {
  94  |     await this.buttonNew.click({ timeout: 10000 });
  95  |     await expect(this.page.getByText("New Lead", { exact: true })).toBeVisible({ timeout: 10000 });
  96  |     await this.h.pause(1000);
  97  |   }
  98  | 
  99  |   async fillLeadForm(lead: LeadData) {
  100 |     await this.textFieldFirstName.fill(lead.firstName);
  101 |     await this.textFieldLastName.fill(lead.lastName);
  102 |     await this.textFieldCompany.fill(lead.company);
  103 |     await this.textFieldTitle.fill(lead.title);
  104 |     await this.textFieldAnnualRevenue.fill(lead.annualRevenue);
  105 |     await this.textFieldPhone.fill(lead.phone);
  106 |     await this.textFieldMobile.fill(lead.mobile);
  107 |     await this.textFieldFax.fill(lead.fax);
  108 |     await this.textFieldEmail.fill(lead.email);
  109 |     await this.textFieldWebsite.fill(lead.website);
  110 |     await this.textFieldNoOfEmployees.fill(lead.numberOfEmployees);
  111 | 
  112 |     await this.fillLeadFormDropdownV2(lead);
  113 |   }
  114 | 
  115 |   async fillLeadTextField(field: Locator, value: string) {
  116 |     await field.fill(value);
  117 |   }
  118 | 
  119 |   async fillLeadFormDropdown(lead: LeadData) {
  120 |     await this.dropdownSalutation.click();
  121 |     await this.page.getByRole("option", { name: lead.salutation }).click();
  122 | 
  123 |     await this.dropdownLeadSource.click();
  124 |     await this.page.getByRole("option", { name: lead.leadSource }).click();
  125 | 
  126 |     await this.dropdownIndustry.click();
  127 |     await this.page.getByRole("option", { name: lead.industry, exact: true }).click();
  128 | 
  129 |     await this.dropdownLeadStatus.click();
  130 |     await this.page.getByRole("option", { name: lead.leadStatus }).click();
  131 | 
  132 |     await this.dropdownRating.click();
  133 |     await this.page.getByRole("option", { name: lead.rating }).click();
  134 |   }
  135 | 
  136 |   private async selectOption(dropdown: Locator, value: string, exact = false) {
  137 |     await dropdown.click();
  138 |     await this.page.getByRole("option", { name: value, exact }).click();
  139 |   }
  140 | 
  141 |   async fillLeadFormDropdownV2(lead: LeadData) {
  142 |     const dropdowns = [
  143 |       { locator: this.dropdownSalutation, value: lead.salutation },
  144 |       { locator: this.dropdownLeadSource, value: lead.leadSource },
  145 |       { locator: this.dropdownIndustry, value: lead.industry, exact: true },
  146 |       { locator: this.dropdownLeadStatus, value: lead.leadStatus },
  147 |       { locator: this.dropdownRating, value: lead.rating },
  148 |     ];
  149 | 
  150 |     for (const dropdown of dropdowns) {
  151 |       await this.selectOption(dropdown.locator, dropdown.value, dropdown.exact);
  152 |     }
  153 |   }
  154 | 
  155 |   async saveLead() {
  156 |     await this.buttonSave.click();
  157 |     await expect(this.page.getByText("was created.")).toBeVisible();
  158 |     await expect(this.page).toHaveURL(this.leadRecordUrl, {
  159 |       timeout: 10000,
  160 |     });
  161 |   }
  162 | 
  163 |   async clickSaveButton() {
  164 |     await this.buttonSave.click();
  165 |   }
  166 | 
  167 |   async verifyUniqueLeadId() {
  168 |     const match = this.page.url().match(this.leadRecordUrl);
  169 |     expect(match).not.toBeNull();
  170 |     expect(match![1]).toHaveLength(18);
  171 |     expect(match![1]).toMatch(/^[A-Za-z0-9]{18}$/);
  172 |   }
  173 | 
  174 |   async deleteLead() {
  175 |     await this.buttonShowMoreActions.click();
  176 |     await this.buttonDelete.click();
  177 |     await expect(this.page.getByText("Are you sure you want to delete this")).toBeVisible();
  178 |     await this.buttonDelete2.click();
> 179 |     await expect(this.page.getByText("was deleted.")).toBeVisible();
      |                                                       ^ Error: expect(locator).toBeVisible() failed
  180 |   }
  181 | 
  182 |   async editLead() {
  183 |     await this.buttonShowMoreActions.click();
  184 |     await this.buttonEdit.click();
  185 |     await expect(this.buttonSaveAndNew).toBeVisible();
  186 |     await expect(this.buttonSave).toBeVisible();
  187 |   }
  188 | 
  189 |   async clickConvertButton() {
  190 |     await this.buttonShowMoreActions.click();
  191 |     await expect(this.buttonConvertMenuItem).toBeVisible({ timeout: 10000 });
  192 |     await this.buttonConvertMenuItem.click();
  193 |     await expect(this.page.getByRole("dialog", { name: "Convert Lead" })).toBeVisible({
  194 |       timeout: 15000,
  195 |     });
  196 |   }
  197 | 
  198 |   async editLeadStatus(newStatus: string) {
  199 |     await this.buttonShowMoreActions.click();
  200 |     await this.buttonEdit.click();
  201 |     await expect(this.dropdownLeadStatus).toBeVisible({ timeout: 10000 });
  202 |     await this.selectOption(this.dropdownLeadStatus, newStatus);
  203 |     await this.buttonSave.click();
  204 |     await expect(this.page.getByText("was saved.")).toBeVisible();
  205 |     await expect(this.page).toHaveURL(this.leadRecordUrl, {
  206 |       timeout: 10000,
  207 |     });
  208 |   }
  209 | 
  210 |   async clickDetailsTab() {
  211 |     await this.tabDetails.click();
  212 |     await expect(this.page.getByText("Lead Owner", { exact: true })).toBeVisible();
  213 |   }
  214 | 
  215 |   private async getTextContent(locator: Locator) {
  216 |     const text = (await locator.allTextContents()).join(" ").replace(/\s+/g, " ").trim();
  217 |     if (text) {
  218 |       return text;
  219 |     }
  220 | 
  221 |     return (await this.page.locator("body").innerText()).replace(/\s+/g, " ").trim();
  222 |   }
  223 | 
  224 |   // Polls until the expected value appears within the locator's text
  225 |   private async assertFieldValue(locator: Locator, expected: string, timeout = 10000) {
  226 |     await expect.poll(async () => this.getTextContent(locator), { timeout }).toContain(expected);
  227 |   }
  228 | 
  229 |   private async resolveFieldValueLocator(label: string, initialValue: Locator, expected: string) {
  230 |     // Return the initial locator if it already contains text
  231 |     const initialText = (await initialValue.allTextContents())
  232 |       .join(" ")
  233 |       .replace(/\s+/g, " ")
  234 |       .trim();
  235 |     if (initialText) {
  236 |       return initialValue;
  237 |     }
  238 | 
  239 |     const labelLocator = this.page.getByText(label, { exact: true }).first();
  240 |     if (await labelLocator.count()) {
  241 |       // Locate the nearest container that holds both the label and its value
  242 |       const container = labelLocator.locator(
  243 |         "xpath=ancestor::*[self::div or self::li or self::tr or self::section or self::article][1]"
  244 |       );
  245 |       // Find non-label elements that may contain the field value
  246 |       const fallbackValue = container.locator(
  247 |         "xpath=.//*[not(self::label) and not(self::span[contains(@class, 'test-id__field-label')]) and normalize-space(.)!='']"
  248 |       );
  249 |       // Exclude elements whose text is only the label
  250 |       const candidate = fallbackValue.filter({ hasNotText: new RegExp(`^${label}$`, "i") }).first();
  251 |       if (await candidate.count()) {
  252 |         return candidate;
  253 |       }
  254 |       // Use the first available value element if no better match exists
  255 |       if (await fallbackValue.count()) {
  256 |         return fallbackValue.first();
  257 |       }
  258 |     }
  259 | 
  260 |     return this.page.locator("body");
  261 |   }
  262 | 
  263 |   /**
  264 |    * Verifies the displayed value of a Salesforce record field based on its label.
  265 |    * Optionally normalizes numeric values (e.g. Phone, Mobile, Fax) to ignore
  266 |    * formatting differences such as spaces, parentheses, or dashes.
  267 |    */
  268 |   async verifyField(label: string, expected: string, normalizeDigits = false) {
  269 |     const field = this.page
  270 |       .locator(`.test-id__field-label:text-is("${label}")`)
  271 |       .locator(
  272 |         "xpath=ancestor::*[contains(concat(' ', normalize-space(@class), ' '), ' slds-form-element ')][1]"
  273 |       );
  274 | 
  275 |     // Retrieve the displayed field value
  276 |     const value = field.locator(".test-id__field-value").first();
  277 |     const resolvedValue = await this.resolveFieldValueLocator(label, value, expected);
  278 | 
  279 |     // Compare only numeric characters for phone-related fields
```